...
def main(argv=None):
    ...
    inputs = {}
    for name in ('registry', 'manifest', 'readme', 'index', 'version', 'controls'):
    ...
    registry, documents = generate(
        ...
        args.code_commit, {name: inputs[name].decode('utf-8') for name in ('readme', 'index', 'version')},
    ...
    outputs = {
        ...
        'version.md': documents['version'], 'CONTEXT_CARD.md': documents['context_card'],
    }
