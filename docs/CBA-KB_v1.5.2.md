...
> **维护决定**：CODE_FROZEN_BY_USER（2026-09-11）；收尾仅修改文档与审计记录
> **验证日期**：2026-09-11
...
- 实际部署回归：**91 passed, 1 skipped**；完整来源验证：**PASS_IMPLEMENTED_TABLE_SCOPE**。唯一跳过是旧八一 midseason 专项来源未放在测试固定路径，MASTER 基线回归在实际部署目录已运行。此前开发工作树的 88 passed / 2 skipped 是历史结果。当前两份 DRAFT-2 合并资料和原始外援 XLSX 已单独执行真实来源验证。
...
1. 用户明确确认 `2020-2021` 和 `2022-2023` 外援 PNG OCR 不需要，本轮不再以此作为等待完成的任务。已发布注册表中的 `DISCOVERED / ocr_deferred` 是历史处理记录，不能据此声称已 OCR 或已导入 PNG；Markdown 转录不等于 PNG 验收。
...
用户在 2026-09-11 确认本次收尾后不再调整代码。因此以上未实施项按已知限制留档，不再作为本版本的持续开发承诺。Gemini 消费复验没有结果，不计为通过；本版本不标记完整 DRAFT-2 或 STABLE。已发布的 v1.5.2-1、事实字节及原始冻结证据链保持原状，后续控制文件清理另记审计。最终结论及消费规则见 [最终收尾说明](v1.5.2-final-closeout-20260911.md)。

2026-09-11 的外部清理已将 r1/r2/r3 候选移入归档目录，并将四份已处理来源移到原始证据目录；Drive 当前 source_registry 的四项 IMPORTED 是发布后的清理状态，与仓库冻结配置中的 EXTRACTED 分属不同时间点。恢复或复验旧发布时必须区分这两份证据，不能覆盖原 plan。
